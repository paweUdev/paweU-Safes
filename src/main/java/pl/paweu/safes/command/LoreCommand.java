...
public class LoreCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if(!(sender instanceof Player)) return sendMessage(sender, "Tej komendy nie mozna uzyc z poziomu konsoli");
        Player player = (Player) sender;
        if(!player.hasPermission("safes.cmd.lore")) return sendMessage(player, "&4&lBlad: &cNie posiadasz permisji &8(&6safes.cmd.lore&8)");
        if(args.length != 1) return sendMessage(sender, this.settings.loreCommandUsage);
        String lore = args[0];
        if(lore.length() < 2 || lore.length() > 16) return sendMessage(sender, this.settings.loreLengthError);
        ItemStack itemStack = player.getItemInHand();
        if(!this.safeManager.isSafe(itemStack)) return sendMessage(player, this.settings.isNotSafeItem);
